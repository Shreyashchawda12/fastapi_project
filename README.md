...
- 🎯 Entry points to execute your program using `python -m <fastapi_project>` or `$ fastapi_project` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/Shreyashchawda12/fastapi_project/branch/main/graph/badge.svg?token=fastapi_project_token_here)](https://codecov.io/gh/Shreyashchawda12/fastapi_project)
[![CI](https://github.com/Shreyashchawda12/fastapi_project/actions/workflows/main.yml/badge.svg)](https://github.com/Shreyashchawda12/fastapi_project/actions/workflows/main.yml)

Awesome fastapi_project created by Shreyashchawda12

## Install

from source
```bash
git clone https://github.com/Shreyashchawda12/fastapi_project fastapi_project
cd fastapi_project
make install
```
...
```bash
pip install fastapi_project
```

## Executing

```bash
$ fastapi_project run --port 8080
```

or

```bash
python -m fastapi_project run --port 8080
```

or

```bash
$ uvicorn fastapi_project:app
```

## CLI

```bash
❯ fastapi_project --help
Usage: fastapi_project [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ fastapi_project create-user --help
Usage: fastapi_project create-user [OPTIONS] USERNAME PASSWORD
...
```bash
fastapi_project create-user admin admin --superuser
```
...
Run with `fastapi_project run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `fastapi_project create-user`.
...
-----------------------------------------------------
fastapi_project/__init__.py              4      0   100%
fastapi_project/app.py                  16      1    94%
fastapi_project/cli.py                  21      0   100%
fastapi_project/config.py                5      0   100%
fastapi_project/db.py                   10      0   100%
fastapi_project/models/__init__.py       0      0   100%
fastapi_project/models/content.py       47      1    98%
fastapi_project/routes/__init__.py      11      0   100%
fastapi_project/routes/content.py       52     25    52%
fastapi_project/routes/security.py      15      1    93%
fastapi_project/routes/user.py          52     26    50%
fastapi_project/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in fastapi_project/default.toml.

### As environment variables
```bash
export fastapi_project_KEY=value
export fastapi_project_KEY="@int 42"
export fastapi_project_KEY="@jinja {{ this.db.uri }}"
export fastapi_project_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
fastapi_project_ENV=production fastapi_project run
```
